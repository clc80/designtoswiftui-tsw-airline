...
struct BookFlightView: View {
    ...
    @State var datePicker: CustomDatePicker = CustomDatePicker()
    ...
    var container: some View {
        Group {
            ScrollView(.vertical, showsIndicators: false) {
                VStack {
                    BookingHeaderView()
                    BookingContentView(isPresented: $isPresented)
                }
                .padding(.bottom, 80)
            }
            .hideNavigationBar()
    ...
    var footer: some View {
        ZStack {
            ...
            NavigationLink(destination: LazyView(TicketListView()), isActive: $isShowingDetailView) {
